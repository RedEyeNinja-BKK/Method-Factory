...
**Date:** 2026-08-03  
**Source:** [Process Engine v1.9.1](https://github.com/RedEyeNinja-BKK/Process-Engine) — commit `240520e`  
**Split point:** Process Engine v1.6.0 (commit `69de7bc`) — the version that produced the [Etsy store case study](https://github.com/RedEyeNinja-BKK/Process-Engine/blob/main/evals/case-study-first-run.md)

---

## Why the split?

Process Engine was built as a prompt-only system. The core skill began at 92 lines (v1.6.0) — a clean pipeline: Orient → Collect → Clarify → Objective → Summary Gate → Route. That version produced a successful end-to-end package for an Etsy store.

What followed was the evaluator era (v1.7.0 → v1.9.1). The core skill grew to 220 lines — most of that growth was enforcement logic: First-Response Discipline rules, governance boilerplate, manifest mechanics, tier systems. The prompts were doing the code's job.

The split separates two concerns:

1. **Process Engine** — stripped back to its v1.6.0 essence. Prompts only, Turnstone-native. The philosophical reference.
2. **Method Factory** — takes the domain knowledge from the full v1.9.1 and adds a code layer for deterministic enforcement, lighter prompts, and platform-agnostic operation.

---

## What migrated

| From Process Engine (v1.9.1) | Purpose in Method Factory |
|---|---|
...
| `.github/workflows/release-gate.yml` | CI foundation |
